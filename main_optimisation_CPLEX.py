...
def main():
    ...
    where_pieces = {}
    mdl = Model(name = 'puzzle-a-day')
    for i, piece in pieces_positions.items():
        where_pieces[i] = mdl.binary_var_list(len(piece), name=f'x_{i}')
        mdl.add_constraint(mdl.sum(where_pieces[i]) == 1)
    ...
    if solution:
        for i in pieces_positions:
            for k in range(len(pieces_positions[i])):
                if where_pieces[i][k].solution_value > 0.5:
